...
namespace {
// workspace variables
enum AsphereTempSpaceType {kCost, kCosmt, kK, kSin2t, kFo, kCostM};
}
...
template<typename DType>
__global__ void AsphereForwardKernel(const Tensor<gpu, 2, DType> x,
...
template<typename DType>
inline void AsphereForward(const Tensor<gpu, 2, DType> &x,
                            const Tensor<gpu, 2, DType> &w,
                            const Tensor<gpu, 1, DType> &label,
  ...
  AsphereForwardKernel<<<dimGrid, dimBlock>>>(x, w, label, x_norm, w_norm, out, k_table, c_table, margin, beta);
}

template<typename DType>
__global__ void AsphereBackwardRequired(const Tensor<gpu, 2, DType> x,
                                         const Tensor<gpu, 2, DType> w,
                                         const Tensor<gpu, 1, DType> label,
                                         const Tensor<gpu, 1, DType> x_norm,
                                         const Tensor<gpu, 1, DType> w_norm,
...
template<typename DType>
__global__ void AsphereBackwardXKernel(const Tensor<gpu, 2, DType> x,
                                        const Tensor<gpu, 2, DType> w,
                                        const Tensor<gpu, 1, DType> label,
                                        const Tensor<gpu, 1, DType> x_norm,
                                        const Tensor<gpu, 1, DType> w_norm,
  ...
  CUDA_KERNEL_LOOP(idx, nthreads) {
    ...
    float norm_df_dx_w_mag = 0.0;
    stat1=cublasSnrm2(handle1,feature_dim,df_dx,1,&norm_df_dx_w_mag);
    if(stat1 != CUBLAS_STATUS_SUCCESS){printf("error code %d, line(%d)\n", stat1, __LINE__);exit(EXIT_FAILURE);}
    norm_df_dx = df_dx/norm_df_dx_w_mag.
    //deepearthgo

    x_grad[i][l] += alpha * o_grad[i][yi] * (norm_df_dx - w[yi][l]);
  }
}

template<typename DType>
__global__ void AsphereBackwardWKernel(const Tensor<gpu, 2, DType> x,
                                        const Tensor<gpu, 2, DType> w,
                                        const Tensor<gpu, 1, DType> label,
                                        const Tensor<gpu, 1, DType> x_norm,
                                        const Tensor<gpu, 1, DType> w_norm,
  ...
  CUDA_KERNEL_LOOP(idx, nthreads) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (yi == j) {
        ...
        float norm_df_dw_j_mag = 0.0;
        stat2=cublasSnrm2(handle2,feature_dim,df_dw_j,1,&norm_df_dw_j_mag);
        if(stat2 != CUBLAS_STATUS_SUCCESS){printf("error code %d, line(%d)\n", stat2, __LINE__);exit(EXIT_FAILURE);}
        norm_df_dw_j = (df_dw_j)/norm_df_dw_j_mag.
        //deepearthgo

        dw += o_grad[i][yi] * (norm_df_dw_j - x[i][l]);
      }
    }
    const DType alpha = 1 / (1 + beta);
    w_grad[j][l] += alpha * dw;
...
template<typename DType>
inline void AsphereBackward(const Tensor<gpu, 2, DType> &x,
                             const Tensor<gpu, 2, DType> &w,
                             const Tensor<gpu, 1, DType> &label,
                             const Tensor<gpu, 1, DType> &x_norm,
                             const Tensor<gpu, 1, DType> &w_norm,
  ...
  dim3 dimBlock(kBaseThreadNum);
  dim3 dimGrid((n + kBaseThreadNum - 1) / kBaseThreadNum);
  AsphereBackwardRequired<<<dimGrid, dimBlock>>>(x, w, label, x_norm, w_norm, workspace,
                                                  k_table, c_table, margin);
  dimGrid.x = ((n * feature_dim + kBaseThreadNum - 1) / kBaseThreadNum);
  AsphereBackwardXKernel<<<dimGrid, dimBlock>>>(x, w, label, x_norm, w_norm, o_grad, x_grad, workspace,
                                                 c_table, margin, beta);
  dimGrid.x = ((m * feature_dim + kBaseThreadNum - 1) / kBaseThreadNum);
  AsphereBackwardWKernel<<<dimGrid, dimBlock>>>(x, w, label, x_norm, w_norm, o_grad, w_grad, workspace,
                                                 c_table, margin, beta);
}
...
inline void AsphereForward(const Tensor<gpu, 2, DType> &x,
                            ...
                            const int margin,
                            const DType beta) {
  cuda::AsphereForward(x, w, label, out, x_norm, w_norm,
...
inline void AsphereBackward(const Tensor<gpu, 2, DType> &x,
                             ...
                             const int margin,
                             const DType beta) {
  cuda::AsphereBackward(x, w, label, x_norm, w_norm, o_grad, x_grad, w_grad, workspace,
                         k_table, c_table, margin, beta);
}
...
template<>
Operator *CreateOp<gpu>(AsphereParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new AsphereOp<gpu, DType>(param);
  })
  return op;
}
